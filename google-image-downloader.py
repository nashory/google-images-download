...
parser.add_argument('-n', '--directory_name', help='dir name to be saved', type=str, required=True)
...
# or download multiple images based on keywords
    ...
    while i < len(search_keyword):
        ...
        time.sleep(0.1)
        items = items + (_images_get_all_items(raw_html, limit=limit))
        ...
        while (k < limit):
            try:
                ...
                if '?' in image_name:
                    image_name = image_name[:image_name.find('?')]
                if ".jpg" in image_name or ".png" in image_name or ".jpeg" in image_name or ".svg" in image_name:
                    output_file = open(os.path.join(main_directory, dir_name, str(k+1)+"_"+image_name), 'wb')
                else:
                    image_name = image_name + ".jpg"
                    output_file = open(os.path.join(main_directory, dir_name, str(k+1)+"_"+image_name), 'wb')
